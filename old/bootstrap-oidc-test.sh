...
APISIX_ADMIN="http://apisix-test:9181"  # service name inside docker network

# Wait for APISIX Admin API
echo "Waiting for APISIX Test Admin API..."
until curl -fsS "$APISIX_ADMIN/apisix/admin/routes" -H "X-API-KEY: $ADMIN_KEY" >/dev/null 2>&1; do
  sleep 1
done

echo "Applying OIDC route from JSON template to test environment..."
...
echo "OIDC route upserted successfully to test environment."