...
configure_consumer_groups() {
    ...
    # Source the consumer groups configuration script
    . /opt/apisix-gateway/scripts/bootstrap/configure-consumer-groups.sh
    configure_consumer_groups "$apisix_admin"
}
...
# Verify consumer groups configuration
verify_consumer_groups() {
    ...
    log_info "Verifying consumer groups configuration at $apisix_admin ($network_context context)..."

    # Check base_user group
    if curl -fsS -H "X-API-KEY: $ADMIN_KEY" "$apisix_admin/consumer_groups/base_user" >/dev/null 2>&1; then
        log_success "base_user consumer group verified"
    else
        log_error "base_user consumer group not found"
        return 1
    fi

    # Check premium_user group
    if curl -fsS -H "X-API-KEY: $ADMIN_KEY" "$apisix_admin/consumer_groups/premium_user" >/dev/null 2>&1; then
        log_success "premium_user consumer group verified"
    else
        log_error "premium_user consumer group not found"
        return 1
    fi

    log_success "Consumer groups configuration verified"
}

# Verify complete configuration (routes and consumer groups)
verify_configuration() {
    log_info "Verifying complete APISIX configuration..."
    ...
    log_success "Complete configuration verification passed"
}
