...
generate_complete_env_file() {
    local provider="${1:-}"
    local environment="${2:-dev}"
    local env_file="$PROJECT_ROOT/config/env/${environment}.env"
    local complete_env_file="$PROJECT_ROOT/config/env/${environment}.complete.env"

    log_debug "Generating complete env file: $complete_env_file"

    # Start with base env file content (host ports, basic config)
    if [[ -f "$env_file" ]]; then
        cp "$env_file" "$complete_env_file"
        echo "" >> "$complete_env_file"
        echo "# Generated variables from shared/provider/secrets configuration" >> "$complete_env_file"
    else
        echo "# Complete environment file for $environment" > "$complete_env_file"
        echo "# Generated by setup_environment" >> "$complete_env_file"
        echo "" >> "$complete_env_file"
    fi

    # Add all resolved environment variables needed by Docker Compose
    cat >> "$complete_env_file" << EOF
...
APISIX_ADMIN_PORT=${APISIX_ADMIN_PORT}
APISIX_ADMIN_API=${APISIX_ADMIN_API}
APISIX_ADMIN_API_CONTAINER=${APISIX_ADMIN_API_CONTAINER}
...
PORTAL_BACKEND_HOST=${PORTAL_BACKEND_HOST}
PORTAL_BACKEND_PORT=${PORTAL_BACKEND_PORT}
BACKEND_HOST=${BACKEND_HOST}

# Legacy Azure AD Variables
...
# Network Context
APISIX_NETWORK_CONTEXT=${APISIX_NETWORK_CONTEXT:-}
EOF

    log_debug "Complete env file generated: $complete_env_file"
}
...
generate_compose_command() {
    ...
    # Set environment file path (use complete generated file)
    export COMPOSE_ENV_FILE="$PROJECT_ROOT/config/env/${environment}.complete.env"
